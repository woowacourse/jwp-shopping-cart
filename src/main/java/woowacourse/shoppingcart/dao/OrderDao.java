...
public class OrderDao {
    ...
    public OrderDao(final JdbcTemplate jdbcTemplate, final DataSource dataSource, final ProductDao productDao) {
    ...
    public Order findById(Long id) {
        ...
        List<OrderProduct> orderProducts = findOrderProductsByOrderId(id);
    ...
    public List<Order> findAllByCustomerId(Long customerId) {
        ...
        return orderIds.stream()
                .map(this::findOrderProductsByOrderId)
                .map(orderProduct -> new Order(orderProduct, customerId))
                .collect(Collectors.toList());
    }

    private List<OrderProduct> findOrderProductsByOrderId(Long id) {
        final String sql = "SELECT product_id, quantity FROM order_product WHERE order_id = ?";
        return jdbcTemplate.query(sql, (rs, rowNum) -> {
            Product product = productDao.findProductById(rs.getLong("product_id"));
