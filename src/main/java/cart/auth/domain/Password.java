...
public class Password {
    ...
    private void validateHavingSpecialLetters(final String password) {
        if (!password.matches(".*[~!@#$%^&*()].*")) {
            throw new InvalidInputException(PASSWORD_NOT_HAVING_SPECIAL_CHARACTER_ERROR);
        }
    }

    private void validateHavingAlphabet(final String password) {
        if (!password.matches(".*[a-zA-Z].*")) {
            throw new InvalidInputException(PASSWORD_NOT_HAVING_ALPHABET_ERROR);
        }
    }

    private void validateHavingNumber(final String password) {
        if (!password.matches(".*[0-9].*")) {
            throw new InvalidInputException(PASSWORD_NOT_HAVING_NUMBER_ERROR);
        }
    }

    private void validateMaxLength(final String password) {
        if (password.length() > 20) {
            throw new InvalidInputException(PASSWORD_OVER_MAX_LENGTH_ERROR);
        }
    }

    private void validateMinLength(final String password) {
        if (password.length() < 8) {
            throw new InvalidInputException(PASSWORD_UNDER_MIN_LENGTH_ERROR);
        }
    }
