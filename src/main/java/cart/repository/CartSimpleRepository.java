...
public class CartSimpleRepository implements CartRepository {
    ...
    List<Cart> carts = Arrays.asList(
            new Cart(new User("rosie@wooteco.com", "1234"),
                    new CartItems(Arrays.asList(new CartItem(1L,
                            Product.from(2L, "hi", "https://avatars.githubusercontent.com/u/61582017?v=4", 100000))))
            )
    );
    ...
    @Override
    public void removeCartItem(Long removalItemId) {
        // TODO: removalItem 삭제하기
        Cart foundCart = carts.stream().filter(element -> element.getCartItems().stream()
                        .anyMatch(cartItem -> cartItem.getId().equals(removalItemId)))
                .findAny()
