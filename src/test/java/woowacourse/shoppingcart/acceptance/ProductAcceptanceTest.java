...
public class ProductAcceptanceTest extends AcceptanceShoppingCartTest {
    ...
    void getProductsWithOutLogin() {
        ...
        조회_응답됨(response);
        상품_목록_포함됨(List.of(productId1, productId2), response);
        상품_목록_카트_아이디_포함됨(null, null, response);
        상품_목록_카트_수량_포함됨(List.of(0, 0), response);
    }
    ...
    void getProducts() {
        ...
        상품_목록_카트_수량_포함됨(List.of(1, 0), response);
    }

    @DisplayName("로그인이 안된 상태로 페이징 처리된 상품 목록을 조회하면, 카트 정보가 cartId: null, quantity: 0이 포함된다.")
    @Test
    void getPageableProductsWithOutLogin() {
        상품_등록되어_있음("치킨", 10_000, "http://example.com/chicken.jpg");
        Long productId2 = 상품_등록되어_있음("맥주", 20_000, "http://example.com/beer.jpg");
        Long productId3 = 상품_등록되어_있음("피자", 15_000, "http://example.com/pizza.jpg");

        ExtractableResponse<Response> response = 비로그인_페이징_상품_목록_조회_요청(2, 1);

        조회_응답됨(response);
        상품_목록_포함됨(List.of(productId2, productId3), response);
        상품_목록_카트_아이디_포함됨(null, null, response);
        상품_목록_카트_수량_포함됨(List.of(0, 0), response);
    }

    @DisplayName("로그인이 된 상태로 페이징 처리된 상품 목록을 조회하면, 카트 정보가 cartId, quantity 정보가 포함된다.")
    @Test
    void getPageableProducts() {
        Long productId1 = 상품_등록되어_있음("치킨", 10_000, "http://example.com/chicken.jpg");
        Long productId2 = 상품_등록되어_있음("맥주", 20_000, "http://example.com/beer.jpg");
        상품_등록되어_있음("피자", 15_000, "http://example.com/pizza.jpg");
        ...
        ExtractableResponse<Response> response = 로그인된_페이징_상품_목록_조회_요청(2, 0, accessToken);
    ...
    public static ExtractableResponse<Response> 비로그인_페이징_상품_목록_조회_요청(int size, int page) {
        return RestAssured
                .given().log().all()
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .when().get("/api/products/pageable?size=" + size + "&page=" + page)
                .then().log().all()
                .extract();
    }
    ...
    public static ExtractableResponse<Response> 로그인된_페이징_상품_목록_조회_요청(int size, int page, String token) {
        return RestAssured
                .given().log().all()
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .header(HttpHeaders.AUTHORIZATION, BEARER + token)
    ...
    public static void 상품_목록_카트_아이디_포함됨(Long cartId1, Long cartId2, ExtractableResponse<Response> response) {
        List<Long> resultCartIds = response.jsonPath().getList(".", ProductResponse.class).stream()
                ...
                .collect(Collectors.toList());
        assertThat(resultCartIds).containsExactly(cartId1, cartId2);
    }

    public static void 상품_목록_카트_수량_포함됨(List<Integer> quantities, ExtractableResponse<Response> response) {
        List<Integer> resultCartQuantities = response.jsonPath().getList(".", ProductResponse.class).stream()
                .map(ProductResponse::getQuantity)
                .collect(Collectors.toList());
        assertThat(resultCartQuantities).containsExactlyElementsOf(quantities);
    }
