...
class ProductServiceTest {
    ...
    void findProductsByCustomer() {
        ...
        given(productDao.findProducts()).willReturn(products);
        given(cartItemDao.findProductIdsByCustomerId(customer.getId())).willReturn(List.of(1L));
        given(cartItemDao.findIdAndQuantityByProductId(any(), any())).willReturn(Optional.of(cart));

        List<ProductResponse> productResponses = productService.findProductsByCustomer(customer);
        assertAll(
