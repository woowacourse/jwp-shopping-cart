...
class CartIntegrationTest {
    ...
    void saveCartAndDeleteCart() throws Exception {
        // given
        addCartProductToMember1();
        int cartId = getCartIdFromMember1();
    ...
    private int getCartIdFromMember1() throws Exception {
        MvcResult mvcResult = mockMvc.perform(get(CART_PATH)
                        .contentType(MediaType.APPLICATION_JSON)
                        .header(AUTHORIZATION, MEMBER_1))
                .andExpect(status().isOk())
                .andReturn();
        return JsonPath.read(mvcResult.getResponse().getContentAsString(), "$.result[0].cartId");
    }

    private void addCartProductToMember1() throws Exception {
