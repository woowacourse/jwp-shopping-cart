...
class ProductNameTest {
    ...
    void createProductName(final String input) {
        assertDoesNotThrow(() -> ProductName.from(input));
    }

    @ParameterizedTest(name = "상품 이름은 공백으로만 이루어질 수 없다.")
    @ValueSource(strings = {"", "   "})
    void createProductNameFailureBlank(final String input) {
    ...
    void createProductNameFailureOverMaxLength(final String input) {
        assertThatThrownBy(() -> ProductName.from(input))
                .isInstanceOf(IllegalArgumentException.class);
    }
}
