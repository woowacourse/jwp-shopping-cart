...
class ProductTest {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"", " ", "  "})
    @DisplayName("constructor : 물품의 이름을 공백으로 입력할 시 IllegalArgumentException가 발생한다.")
    void test_of_IllegalArgumentException_name(final String name) throws Exception {
        ...
        //when & then
        Assertions.assertThatThrownBy(() -> new Product(name, price, imageUrl))
    ...
    @DisplayName("constructor : 물품의 가격을 음수나 0으로 입력할 시 IllegalArgumentException가 발생한다.")
    ...
    @ParameterizedTest
    @ValueSource(strings = {"", " ", "  "})
    @DisplayName("constructor : 물품의 이미지 url을 공백으로 입력할 시 IllegalArgumentException가 발생한다.")
