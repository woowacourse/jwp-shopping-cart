...
class CartProductApiEndToEndTest {
    ...
    @Autowired
    public CartProductApiEndToEndTest(final MemberDao memberDao, final ProductDao productDao, final CartDao cartDao) {
    ...
    void cart_POST로_유효한_상품_추가_성공_테스트() {
        final String email = "a@a.com";
        final String password = "password1";
        final long memberId = memberDao.save(new Member(email, password));
        long pizzaId = productDao.save(new Product("pizza", 20000, "https://a.com"));
    ...
    void cart_DELETE_정상_동작_테스트() {
        ...
        memberDao.save(new Member(email, password));
        final PostCartRequest postCartRequest = new PostCartRequest(2L);
        long cartId = cartDao.save(new Cart(1, 1));
