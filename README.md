...
- [x] 장바구니 기능 구현
  ...
  - [x] 장바구니에 담긴 상품 제거
  - [x] 장바구니 목록 조회
  - [x] 사용자 정보는 Header의 `Authorization` 필드를 사용해 인증 처리한다. 인증 처리는 Basic 방식으로 처리한다.
- [x] 장바구니 페이지 연동
  - [x] `index` 페이지에서 담기 버튼을 통해 상품을 장바구니에 추가할 수 있다.
  - [x] `/cart`로 접근할 경우 장바구니 페이지를 조회할 수 있다.
  - [x] 장바구니 목록을 확인하고 상품을 제거하는 기능을 동작하게 만든다.
